...
describe('Auto Layout', () => {
  ...
  describe('text measurement', () => {
    ...
    test('WIDTH_AND_HEIGHT text editing expands parent hug frame', () => {
      const graph = new SceneGraph()
      const pid = pageId(graph)

      // Hug frame containing WIDTH_AND_HEIGHT text
      const parent = autoFrame(graph, pid, {
        width: 100,
        height: 50,
        layoutMode: 'HORIZONTAL',
        primaryAxisSizing: 'HUG' as const,
        counterAxisSizing: 'HUG' as const,
      ...
      setTextMeasurer((node) => {
        if (node.type === 'TEXT' && node.textAutoResize === 'WIDTH_AND_HEIGHT') {
          // Simulate longer text measurement
          return { width: node.text.length * 8.4, height: 20 }
        }
        return null
      })
      ...
      const initialParent = graph.getNode(parent.id)!
      const initialText = graph.getNode(text.id)!

      // Text should have been measured (8.4 * 2 = 16.8 -> ceil = 17)
      expect(initialText.width).toBe(17)
      // Parent hug frame should be text width + padding
      const initialParentWidth = initialParent.width

      // Simulate text edit: update text and recompute layout
      graph.updateNode(text.id, { text: 'Hello World!' })
      computeAllLayouts(graph)

      const updatedParent = graph.getNode(parent.id)!
      const updatedText = graph.getNode(text.id)!

      // Text should be wider after edit (8.4 * 12 = 100.8 -> ceil = 101)
      expect(updatedText.width).toBe(101)
      // Parent hug frame should expand to fit new text width + padding
      expect(updatedParent.width).toBe(101 + 10 + 10) // 121
      expect(updatedParent.width).toBeGreaterThan(initialParentWidth)
